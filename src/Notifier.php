...
trait Notifier
{
    ...
    {
        $template = self::notificationTemplates()
            ->forKey($key)
            ->forChannel($channel)
            ->forLang($lang)
            ->first();
        ...
        if (blank($template)) {
            $template = self::notificationTemplates()
                ->forKey($key)
                ->forChannel($channel)
                ->forLang(config("model-notification.fallback_lang"))
                ->first();
        }
    ...
    public function getFile($key, $lang, $channel, $file_path = true): ?string
    {
        ...
        if (!$template || !$template->with_file) {
            return null;
        }
    ...
    {
        $resolver = new \Abather\ModelNotification\Resolvers\VariableResolver([
            new \Abather\ModelNotification\Resolvers\SimpleVariableResolver(),
            new \Abather\ModelNotification\Resolvers\RelationshipVariableResolver(),
            new \Abather\ModelNotification\Resolvers\MethodVariableResolver(),
        ]);

        return $resolver->resolve($text, $this, $key, $lang, $channel);
    }
